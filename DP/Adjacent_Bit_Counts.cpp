...
(out of 2ⁿ) that satisfy AdjBC(x) = k. For example, for 5 bit strings, there are 6 ways of getting AdjBC(x) = 2:
...
the number of n-bit strings with adjacent bit count equal to k. As answer can be very large print your answer modulo 10^9+7.
    ...
    for(ll z=1;z<=t;z++)
    {
        ...
        for(ll i=2;i<=size;i++)
        {
                ...
                {   
                    ...
                    {
                    	dp[i][j][1]=(dp[i-1][j][0]%MOD+dp[i-1][j-1][1]%MOD)%MOD;
                    	dp[i][j][0]=(dp[i-1][j][0]%MOD+dp[i-1][j][1]%MOD)%MOD;
                    }
                }
            
        }
        cout<<z<<" "<<(dp[size][k][0]%MOD+dp[size][k][1]%MOD)%MOD<<endl;
	}
